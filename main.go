...
func handleCallDial(w http.ResponseWriter, r *http.Request) {
	...
	writeJSON(w, map[string]string{"status": "dialing"})
}
...
func handleAutoStart(w http.ResponseWriter, r *http.Request) {
	...
	case "GET":
		cfg := GetConfig()
		writeJSON(w, map[string]bool{"auto_start": cfg.AutoStart})
	case "POST":
		var req struct {
		...
		addLog("info", "自启动设置已更新", fmt.Sprintf("%v", req.AutoStart))
		writeJSON(w, map[string]string{"status": "saved"})
	default:
		writeJSON(w, map[string]string{"error": "method not allowed"})
	}
...
func main() {
	...
	log.Printf("运行中 | http://localhost:%s | Ctrl+C 退出", serverPort)
	<-sigCh
